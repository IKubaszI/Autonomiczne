...
void setup() {
  ...
  pinMode(PWMB, OUTPUT); analogWrite(PWMB, 0); // Bezpiecznie: zatrzymany
  pinMode(AIN_1, OUTPUT); digitalWrite(AIN_1, LOW);
  pinMode(AIN_2, OUTPUT); digitalWrite(AIN_2, HIGH);
  pinMode(PWMA, OUTPUT); analogWrite(PWMA, 0); // Bezpiecznie: zatrzymany

  pinMode(SPECIAL_PIN, OUTPUT);
  digitalWrite(SPECIAL_PIN, LOW);

  // --- Bezpieczne ustawienie trybu na start ---
  mode = MODE_DISABLED;
  Stop(); 
